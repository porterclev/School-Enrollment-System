...
def add_department(session: Session):
    ...
    :return:        None
    """
    unique_abbreviation = 0
    unique_name = 0
    unique_chair = 0
    unique_building_and_office = 0
    unique_description = 0
    while not unique_name or not unique_abbreviation or not unique_chair or not unique_building_and_office or not unique_description:
        deptName = input("Department's full name--> ")
        dept_name_check: int = session.query(Department).filter(Department.name == deptName).count()    
        unique_name = dept_name_check == 0
        if not unique_name:
            print("We already have a department by that name. Try again.")
            continue

        deptAbbreviation = input("Department's Abbreviation--> ")
        dept_abbreviation_check: int = session.query(Department).filter(Department.abbreviation == deptAbbreviation).count()
        unique_abbreviation = dept_abbreviation_check == 0
        if not unique_abbreviation:
            print("We already have a department with that abbreviation. Try again.")
            continue

        deptChair = input("Department's Chairperson--> ")
        dept_chair_check: int = session.query(Department).filter(Department.chairName == deptChair).count()
        unique_chair = dept_chair_check == 0
        if not unique_chair:
            print("We already have a department with that chairperson. Try again.")
            continue

        deptBuilding = input("Building--> ")  
        deptOffice = input("Office Number--> ")
        dept_building_and_office_check: int = session.query(Department).filter(Department.building == deptBuilding,
                                                                    Department.office == deptOffice).count()
        unique_building_and_office = dept_building_and_office_check == 0
        if not unique_building_and_office:
            print("We already have a department with that room. Try again.")
            continue

        deptDescription = input("New Department's Description--> ")
        dept_description_check: int = session.query(Department).filter(Department.description == deptDescription).count()
        unique_description = dept_description_check == 0
        if unique_description == 0:
            print("We already have a department with that description. Try again.")
            continue

    newDepartment = Department(deptName, deptAbbreviation, deptChair, deptBuilding, deptOffice, deptDescription)
...
def add_section(session):
    ...
    unique_01: bool = False 
    unique_02: bool = False
    number: int = -1
    year: int = -1
    semester: str = ''
    room: int = -1
    ...
    while not unique_01 or not unique_02:
        ...
        unique_01_count: int = session.query(Section).filter(Section.sectionYear == year, 
                                                             Section.semester == semester,
                                                             Section.schedule == schedule,
                                                             Section.startTime == startTime,
                                                             Section.building == building,
                                                             Section.room == room).count()
        unique_01 = unique_01_count == 0
        if not unique_01:
            print("There is already section at this time")
            continue

        unique_02_count: int = session.query(Section).filter(Section.sectionYear == year, 
                                                             Section.semester == semester,
                                                             Section.schedule == schedule,
                                                             Section.startTime == startTime,
                                                             Section.instructor == instructor).count()
        unique_02 = unique_02_count == 0
        if not unique_02:
            print("There is already an instructor for that section")
            continue
        
    section = Section(course, year, number, semester, room, building, schedule, startTime, instructor)
    session.add(section)
...
def add_student_to_section(sess):
    student: Student = select_student(sess)
    section: Section = select_section(sess)
    student_section_count: int = sess.query(Enrollment).filter(Enrollment.departmentAbbreviation == section.departmentAbbreviation,
...
def add_section_to_student(sess):
    section: Section = select_section(sess)
    student: Student = select_student(sess)
    student_section_count: int = sess.query(Enrollment).filter(Enrollment.departmentAbbreviation == section.departmentAbbreviation,
                                                            Enrollment.courseNumber == section.courseNumber,
...
def select_section(sess) -> Section:
    ...
    section_schedule: str = ''
    section_start_time: int = -1
    section_instructor: str = ''
    course_number: int = -1
    while not found:
        section_year = int(input("Enter the year that this takes place -> "))
        section_semester = input("Enter the semester this takes place -> ")
        section_schedule = input("Enter the schedule -> ")
        section_start_time = int(input("Enter the start time --> "))
        section_instructor = input("Enter the instructor -> ")

        section_count: int = sess.query(Section).filter(Section.sectionYear == section_year,
                                                        Section.semester == section_semester,
                                                        Section.schedule == section_schedule,
                                                        Section.startTime == section_start_time,
                                                        Section.instructor == section_instructor).count()
        found = section_count == 1
        if not found:
            print("There is no section in the course by such specifications. Try again.")
    section: Section = sess.query(Section).filter(Section.sectionYear == section_year,
                                                    Section.semester == section_semester,
                                                        Section.schedule == section_schedule,
                                                        Section.startTime == section_start_time,
                                                        Section.instructor == section_instructor).first()
    return section


def delete_student(session: Session):
    print("deleting a student")
    oldStudent = select_student(session)
    n_sections = session.query(Enrollment).filter(
        Enrollment.studentID == oldStudent.studentID).count()
    if n_sections > 0:
        print(f"Sorry, there are {n_sections} sections with this student.  Delete them first, "
...
def delete_course(session: Session):
    ...
    n_sections = session.query(Section).filter(
...
def delete_student_enrollment(sess):
    print("Prompting you for the student and the section that they no longer have.")
    student: Student = select_student(sess)
    section: Section = select_section(sess)
    x = student.remove_section(section)
    if x == False:
        print("The student is not in the section.")


def delete_section_enrollment(sess):
    print("Prompting you for the section and the student that they no longer have.")
    section: Section = select_section(sess)
    student: Student = select_student(sess)
    x = section.remove_student(student)
    if x == False:
        print("The student is not in the section.")
...
def delete_section(session: Session):
    ...
    oldSection = select_section(session)
    n_students = session.query(Enrollment).filter(
        Enrollment.departmentAbbreviation == oldSection.departmentAbbreviation,
        Enrollment.courseNumber == oldSection.courseNumber,
        Enrollment.sectionYear == oldSection.sectionYear,
        Enrollment.semester == oldSection.semester).count()
    if n_students > 0:
...
def list_section(sess):
    sections: [Section] = sess.query(Section).order_by(Section.departmentAbbreviation, Section.courseNumber)
...
def list_student_section(sess: Session):
    ...
    recs = sess.query(Student).join(Enrollment, Student.studentID == Enrollment.studentID).join(
        Section).filter(Student.studentID == student.studentID).add_columns(Student.lastName,
        Student.firstName, Section.departmentAbbreviation, Section.courseNumber, Section.sectionNumber).all()
    for stu in recs:
        print(f"Student name: {stu.lastName}, {stu.firstName}, Department: {stu.departmentAbbreviation}, Course: {stu.courseNumber}, Section: {stu.sectionNumber}")


def list_section_student(sess: Session):
    section: Section = select_section(sess)
    recs = sess.query(Student).join(Enrollment, Student.studentID == Enrollment.studentID).join(Section).filter(
        Section.departmentAbbreviation == section.departmentAbbreviation, Section.courseNumber == section.courseNumber, Section.sectionNumber == section.sectionNumber
        ).add_columns(Student.lastName, Student.firstName, Section.departmentAbbreviation,
        Section.courseNumber, Section.sectionNumber).all()
    for stu in recs:
        print(f"Department: {stu.departmentAbbreviation}, Course: {stu.courseNumber}, Section: {stu.sectionNumber}, Student name: {stu.lastName}, {stu.firstName}")
...
def boilerplate(sess):
    ...
    :return:        None
    """
    department: Department = Department('Computer Science Computer Engineering', 'CECS', 'Computer', 'ECS', 5, "COMPUTERS RULE")
    course1: Course = Course(department, 1, 'BASED', 'BASED', 3)
    course2: Course = Course(department, 2, 'AWE', 'AWE', 4)
    section1: Section = Section(course1, 2023, 1, "Fall", 1, "ECS", "MW", 8, "BASED")
    section2: Section = Section(course2, 2023, 2, "Spring", 2, "ECS", "TuTh", 10, "AWE")
    section3: Section = Section(course1, 2023, 3, "Spring", 3, "ECS", "MWF", 12, "WASTE")
    student1: Student = Student('COOL', 'COOL', 'COOL@gmail.com')
    student2: Student = Student('RAD', 'RAD', 'RADgmail.com')
    student3: Student = Student('BYE', 'BYE', 'BYE@gmail.com')
    sess.add(department)
    sess.add(course1)
    sess.add(course2)
    sess.add(section1)
